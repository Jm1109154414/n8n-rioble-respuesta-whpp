...
DEFAULT_SELLER_NAME
SELLER_NOTIFY_PHONE
SELLER_NOTIFY_PHONES
SELLER_NOTIFY_PHONE_NUMBER_ID
REAL_ESTATE_BOOKING_LINK
```

`SELLER_NOTIFY_PHONES` acepta uno o varios WhatsApp separados por coma. Si no existe, el workflow usa `SELLER_NOTIFY_PHONE`, luego `DEFAULT_SELLER_PHONE`.
